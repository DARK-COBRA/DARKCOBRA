...
async def gmute(userbot): 
   ...
   me = await userbot.client.get_me() ; await friday.edit(f"Gmuting This Nigga") ; my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id) ; my_username = f"@{me.username}" if me.username else my_mention ; chat = await userbot.get_chat() ; a = b = 0
