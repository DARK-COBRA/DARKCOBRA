...
async def sunny(text):
        ...
        with open("danish.png", "wb") as f:
            f.write(requests.get(cobra).content)
        img = Image.open("danish.png").convert("RGB")
...
async def moditweet(text):
        ...
        if not hburl:
            return  "check syntax once more"
        with open("dc.png", "wb") as f:
            f.write(requests.get(hehe).content)
...
async def sunny(event):
    ...
    if not text:
        if event.is_reply and not reply_to_id.media:
            ...
            await event.edit("`Send you text to Sunny so she can tweet.`")
            return
...
async def teletweet(danish):
    # """Creates random anime sticker!"""
    what = danish.pattern_match.group(1)
    if not what:
        if danish.is_reply:
            what = (await danish.get_reply_message()).message
        else:
            await danish.edit( "Tweets must contain some text, pero!")
            return
...
CMD_HELP.update(
    {
        "teletweet": ".tweet <username>.<tweet>"
